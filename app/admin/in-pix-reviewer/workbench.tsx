...
export function InPixReviewerWorkbench() {
 ...
 useEffect(()=>{void fetch("/api/admin/in-pix-reviewer/queue",{cache:"no-store"}).then(async r=>{const body=await r.json().catch(()=>({})) as QueueResponse;if(!r.ok)throw new Error(body.detail||body.error||"Queue load failed");const items=body.queue||[];setQueue(items);setActiveIndex(0);setBlks(decodeNotes(items[0]?.structure_notes||null));setStatus(items.length?"Listen through the whole IN-PIX, then map every lyric BLK.":"No pending IN-PIX structure reviews.");}).catch((e:unknown)=>setStatus(e instanceof Error?e.message:"Queue load failed"));},[]);
 const active=queue[activeIndex]||null; const audioSrc=useMemo(()=>active?`/api/admin/in-pix-reviewer/audio/${encodeURIComponent(String(active.id))}`:"",[active]);
