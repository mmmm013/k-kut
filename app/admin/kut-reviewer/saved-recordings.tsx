...
type Item = { record_key: string; title: string; parent_title: string; product_layer: string; inventory_state: string; object_exists: boolean; audio_available: boolean; source_table: string; source_record_id: string; capture_start_sec: number | null; capture_end_sec: number | null };
...
export function SavedRecordings() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/kut-reviewer/saved", { signal: controller.signal }).then(async response => {
      ...
      setItems(body.items); setDecisions(body.decisions); setActive(body.items.find((item: Item) => item.audio_available) || body.items[0] || null);
      setStatus(`${body.total} existing inventory records · ${body.linkedAudio} saved recording links`);
  ...
  return <section className="rounded-xl border border-amber-300/30 bg-stone-950 p-5 text-stone-100">
    ...
    <div className="grid gap-4 md:grid-cols-[320px_1fr]">
      <div className="max-h-[65vh] overflow-auto">{items.filter(item => `${item.title} ${item.parent_title}`.toLowerCase().includes(query.toLowerCase())).map(item => <button key={item.record_key} aria-pressed={active?.record_key === item.record_key} onClick={() => { setActive(item); setPlayed(false); }} className="mb-2 block w-full rounded border border-stone-700 p-3 text-left">
        <b>{item.parent_title || item.title}</b><br /><span>{item.title}</span><br /><small>{item.audio_available ? "Stored recording linked" : "Recording link recovery pending"}</small>
      </button>)}</div>
