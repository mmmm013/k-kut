...
const fail = (message) => {
  throw new Error(`CURRENT-II AUTHORITY AUDIT FAIL: ${message}`);
};

const bridgeData = parse(
...
const discoveryBoundary = parse(
  "config/current-ii-discovery-boundary.v1.json",
);
const rollbackSql = read(
  "supabase/rollback-only/current_ii_authority_containment_v1.sql",
);
...
for (const record of canary.records || []) {
  ...
  if (record.inventory_family !== expected.inventoryFamily) {
    fail(`canary product/inventory mismatch ${record.ii_id}`);
  }
  if (record.price_cents !== expected.priceCents) {
    fail(`canary product/price mismatch ${record.ii_id}`);
  }
}
...
const eligible = (bridgeData.records || []).filter((record) => {
  ...
  return Boolean(
    stage &&
      stage.product_family === record.product_family &&
      stage.inventory_family === record.inventory_family &&
      stage.price_cents === record.price_cents &&
...
for (const forbidden of [
  ...
  "/storage/v1/object/public/ii-delivery/catalog/public-ii-catalog.json",
]) {
...
for (const [name, source] of [
  ["direct K-KUT route", directK],
  ["direct mini-KUT route", directMk],
]) {
  if (!source.includes("findApprovedPublicOptionByAnyId")) {
    fail(`${name} lacks current-II lookup`);
  }
  for (const forbidden of ["createClient", "k_kut_audio_qc", "m_kut_assets"]) {
    if (source.includes(forbidden)) fail(`${name} retains ${forbidden}`);
  }
}

for (const [name, source] of [
  ["HUG delivery page", hugPage],
  ["HUG delivery API", hugApi],
]) {
  if (!source.includes("findApprovedPublicOptionByInventoryId")) {
    fail(`${name} lacks current-II lookup`);
  }
  if (source.includes("k_kut_audio_qc")) {
    fail(`${name} still reads legacy playable authority`);
  }
}
...
for (const required of [
  ...
  if (!fulfillment.includes(required)) {
    fail(`4PE fulfillment missing ${required}`);
  }
}
for (const required of [
...
if (webhook.includes("fulfillmentQueueStatus = record.selected_hug_id")) {
  fail("payment_intent.succeeded can create duplicate fulfillment");
}
if (
  !webhook.includes(
  ...
  fail("Stripe webhook lacks single fulfillment-event authority");
}

if (discovery.includes('candidate_generation_status: "ready_for_batch_candidate_generation"')) {
  fail("legacy repo discovery still marks candidates ready");
}
if (discoveryBoundary.repo_keyword_discovery_is_authority !== false) {
  fail("repo keyword discovery remains authority");
}
for (const requiredPrefix of [
  "data/gpmc-sensory/batch-scale",
  "data/hugz",
  "data/ii-delivery-registry",
  "data/production",
  "data/publication-bridge",
  "data/sentimeant",
  "public",
]) {
  if (!discoveryBoundary.excluded_path_prefixes.includes(requiredPrefix)) {
    fail(`discovery boundary missing ${requiredPrefix}`);
  }
}
...
for (const required of [
  ...
  "remaining_public_storage_policy_count",
  "raise exception",
  "rollback;",
]) {
  if (!rollbackSql.toLowerCase().includes(required)) {
    fail(`rollback-only SQL missing ${required}`);
  }
}
...
console.log("LEGACY DIRECT ROUTE AUTHORITY: BLOCKED IN PREPARED CODE");
console.log("LEGACY CHECKOUT CATALOG AUTHORITY: REMOVED IN PREPARED CODE");
console.log("LEGACY FULFILLMENT AUTHORITY: BLOCKED IN PREPARED CODE");
