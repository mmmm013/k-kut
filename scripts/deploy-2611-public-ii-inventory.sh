...
cleanup() {
  rm -f "$NODE_SCRIPT"
  if [[ "$MOVED_LOCAL_ENV" == "1" && -f "$LOCAL_ENV_HOLD" ]]; then
    mv "$LOCAL_ENV_HOLD" "$LOCAL_ENV"
    echo "LOCAL .env.local RESTORED"
  fi
}
...
if [[ -f "$LOCAL_ENV" ]]; then
  mv "$LOCAL_ENV" "$LOCAL_ENV_HOLD"
  MOVED_LOCAL_ENV=1
  echo "BROKEN LOCAL .env.local TEMPORARILY MOVED OUT OF THE WAY"
fi
...
function truthy(value) {
  return ["1", "true", "yes", "pass", "passed", "present", "verified", "at_end", "end"].includes(
...
function parseCsv(text) {
  ...
  const nonblank = rows.filter((values) => values.some((value) => String(value).trim()));
  ...
  return nonblank.slice(1).map((values, index) => {
    if (values.length !== headers.length) {
      stop(`deployment map row ${index + 2} has ${values.length} columns; expected ${headers.length}`);
    }
    return Object.fromEntries(headers.map((header, col) => [header, String(values[col] ?? "").trim()]));
  });
}
...
if (rows.length !== 2611) stop(`deployment map has ${rows.length} rows; expected 2611`);
...
const jobs = rows.map((row, index) => {
  ...
  if (!sourcePath || !fs.existsSync(sourcePath)) stop(`${inventoryId}: local capsule missing: ${sourcePath}`);
  const stat = fs.statSync(sourcePath);
  if (!stat.isFile() || stat.size === 0) stop(`${inventoryId}: local capsule is empty`);
  if (!sourcePath.toLowerCase().endsWith(".mp3")) stop(`${inventoryId}: capsule is not MP3`);
  if (row.local_capsule_size_bytes && Number(row.local_capsule_size_bytes) !== stat.size) {
    stop(`${inventoryId}: local size no longer matches deployment map`);
  }
  if (row.signature_audio_logo_integral_at_end && !truthy(row.signature_audio_logo_integral_at_end)) {
...
if (new Set(jobs.map((job) => job.inventoryId)).size !== 2611) stop("inventory IDs are not unique");
if (new Set(jobs.map((job) => job.objectPath)).size !== 2611) stop("public object paths are not unique");
...
if (!supabaseUrl) stop("Vercel production Supabase URL is missing");
if (!keyCandidates.length) stop("Vercel production Supabase server key is missing");

const [keyName, serverKey] = keyCandidates[0];
console.log("GPMx 2611 II STORAGE DEPLOYMENT");
console.log("=".repeat(76));
console.log("DEPLOYMENT MAP ROWS:", rows.length);
console.log("SERVER KEY SOURCE:", keyName);
...
if (bucketError) stop(`production Supabase authentication failed: ${bucketError.message}`);
...
function appendState(job, disposition) {
  const publicUrl = supabase.storage.from(BUCKET).getPublicUrl(job.objectPath).data.publicUrl;
  fs.appendFileSync(STATE, JSON.stringify({
...
function progress() {
  ...
  if (processed === 1 || processed % 25 === 0 || processed === jobs.length) {
    console.log(`PROGRESS ${processed}/${jobs.length} | uploaded=${uploadedNow} | existing=${alreadyPresent} | resumed=${resumeSkipped}`);
  }
}
...
async function worker() {
  while (true) {
    ...
    if (error) {
      ...
      if (!(status === "409" || /already exists|duplicate|resource exists/i.test(message))) {
...
async function listAll() {
  ...
  while (true) {
    ...
    const page = data || [];
    found.push(...page.filter((item) => item.name.toLowerCase().endsWith(".mp3")));
...
const uploadedObjects = await listAll();
if (uploadedObjects.length !== 2611) {
  stop(`public storage contains ${uploadedObjects.length}/2611 MP3s; run this same command again to resume`);
}
...
const catalogRecords = jobs.map((job) => ({
  ...
  public_audio_url: supabase.storage.from(BUCKET).getPublicUrl(job.objectPath).data.publicUrl,
  object_path: job.objectPath,
  local_capsule_sha256: job.row.local_capsule_sha256,
  local_capsule_size_bytes: job.sizeBytes,
  signature_audio_logo_integral_at_end: job.row.signature_audio_logo_integral_at_end,
...
for (const [objectPath, localPath] of [
  ...
  const { error } = await supabase.storage.from(BUCKET).upload(
    ...
    { contentType: objectPath.endsWith(".json") ? "application/json" : "text/csv", cacheControl: "300", upsert: true }
  );
  if (error) stop(`catalog publication failed for ${objectPath}: ${error.message}`);
}

for (const job of [jobs[0], jobs[Math.floor(jobs.length / 2)], jobs[jobs.length - 1]]) {
  const url = supabase.storage.from(BUCKET).getPublicUrl(job.objectPath).data.publicUrl;
  const response = await fetch(url, { headers: { Range: "bytes=0-0" } });
  if (!response.ok && response.status !== 206) stop(`${job.inventoryId}: public URL returned HTTP ${response.status}`);
}

const catalogUrl = supabase.storage.from(BUCKET).getPublicUrl("catalog/public-ii-catalog.json").data.publicUrl;
const summary = [
...
env \
  ...
  "${VERCEL_COMMAND[@]}" env run -e production -- node "$NODE_SCRIPT" \
  2>&1 | tee "$OUT/00_PUBLIC_UPLOAD_TERMINAL_LOG.txt"
STATUS=${pipestatus[1]}
set -e

if [[ "$STATUS" -ne 0 ]]; then
  echo "UPLOAD STOPPED. Run this same command again after the printed STOP reason is corrected. Completed uploads are preserved."
  exit "$STATUS"
fi
