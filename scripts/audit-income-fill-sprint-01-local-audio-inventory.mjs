...
const p = "data/gpmc-sensory/income-fill/audio-discovery/income-fill-sprint-01-local-audio-inventory.json";
...
console.log("INCOME FILL SPRINT 01 LOCAL AUDIO INVENTORY AUDIT");
...
if (!fs.existsSync(p)) {
  ...
  if (data.status !== "income_fill_sprint_01_local_audio_inventory") fail("Wrong status.");
  if (data.public_status !== "not_public") fail("Inventory must remain not_public.");
...
if (failed) {
  console.error("INCOME FILL SPRINT 01 LOCAL AUDIO INVENTORY AUDIT: FAIL");
  process.exit(1);
}

console.log("INCOME FILL SPRINT 01 LOCAL AUDIO INVENTORY AUDIT: PASS");
