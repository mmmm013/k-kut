...
if (packet.transition?.tail_padding_before_twinkle_seconds !== 1.0) stop("canary transition must use 1.0-second end padding");
if (packet.transition?.validation_command !== "npm run audit:thank-you:twinkle-loudness") stop("canary loudness validation command missing");
if (packet.transition?.status !== "PREVIEW_MEASUREMENT_REQUIRED_BEFORE_PROMOTION") stop("canary loudness promotion gate missing");
