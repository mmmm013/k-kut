...
## Shared matching language

The customer-side MGS and music-side MGS must use the same controlled glossary for:
...
- exact Start TP and End TP boundaries;
- absence of material contradiction;
- authorized source and rights evidence;
- required Twinkle-at-end proof;
- delivery authority.

A candidate may fit multiple themes. Each fit must state how strongly it fits, why it fits, and any concern or contradiction.

Anything without a defensible theme fit remains:
...
The system presents exactly three explained candidates at a time.

Each candidate explanation must identify:
...
The user must be able to ask for different candidates, reject all, refine the sentence, change direction, or start over.
